...
def candidate_models(seed):
    return {
        "extra_trees": ExtraTreesRegressor(n_estimators=360, min_samples_leaf=2, max_features=0.85, random_state=seed, n_jobs=-1),
        "random_forest": RandomForestRegressor(n_estimators=320, min_samples_leaf=2, max_features=0.85, random_state=seed, n_jobs=-1),
        "hist_gradient_boosting": MultiOutputRegressor(HistGradientBoostingRegressor(max_iter=260, learning_rate=0.05, l2_regularization=0.08, random_state=seed)),
    }
...
def training_profile(df):
    return {
        "numericRanges": {column: {"min": float(df[column].min()), "max": float(df[column].max())} for column in NUMERIC},
        "categories": {column: sorted(df[column].dropna().astype(str).unique().tolist()) for column in CATEGORICAL},
    }
...
def train(regenerate=False, rows=None, seed=42):
    ...
    candidate_metrics = [cross_validate_model(name, model, x_train, y_train, seed) for name, model in candidates.items()]
    ...
    info = {
        ...
        "metrics": {"mae": metrics["mae"], "r2": metrics["r2"]},
