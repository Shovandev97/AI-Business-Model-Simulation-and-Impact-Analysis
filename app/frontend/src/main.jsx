...
function CreatePage({ draft, setDraft, referenceData, formError, saveScenario, loading }) {
  ...
  return <section className="panel createScenarioPage"><h2>Scenario Builder</h2><p>Create a commercial model scenario with stable enterprise parameters for O2C impact analysis.</p>{formError && <div className="message error">{formError}</div>}<div className="createScenarioForm">
    <Field label="Scenario Name" value={draft.name} onChange={(v) => update('name', v)}/><Field label="Industry" value={draft.industry} onChange={(v) => update('industry', v)} options={fields.industries}/><Field wide label="Description" value={draft.description} onChange={(v) => update('description', v)} textarea/><Field label="Customer Segment" value={draft.customerSegment} onChange={(v) => update('customerSegment', v)} options={fields.customerSegments}/><Field label="Region" value={draft.region} onChange={(v) => update('region', v)} options={fields.regions}/>
    <Field label="Business Model Type" value={draft.businessModelType} onChange={(v) => update('businessModelType', v)} options={fields.businessModelTypes}/><Field label="Pricing Type" value={draft.pricingType} onChange={(v) => update('pricingType', v)} options={fields.pricingTypes}/><Field label="Contract Term" value={draft.contractTerm} onChange={(v) => update('contractTerm', v)} options={['12 Months','24 Months','36 Months','No Fixed Contract']}/><Field label="Bundle Type" value={draft.bundleType} onChange={(v) => update('bundleType', v)} options={fields.bundleTypes}/>
    <Field label="Billing Frequency" value={draft.billingFrequency} onChange={(v) => update('billingFrequency', v)} options={fields.billingFrequencies}/><Field label="Funding Model" value={draft.fundingModel} onChange={(v) => update('fundingModel', v)} options={fields.fundingModels}/>
    <Field label="Transaction Volume" value={draft.transactionVolume} onChange={(v) => update('transactionVolume', v)} placeholder="Numeric override"/><Field label="Expected Revenue" value={draft.expectedRevenue} onChange={(v) => update('expectedRevenue', v)} placeholder="Numeric override"/><Field label="Integration Count" value={draft.integrationCount} onChange={(v) => update('integrationCount', v)} type="number"/><Field label="Process Complexity" value={draft.processComplexityBand} onChange={(v) => update('processComplexityBand', v)} options={(fields.processComplexities || []).map((x) => String(x.value))}/>
    <Field wide label="Compliance Regions" value={draft.complianceRegionsText} onChange={(v) => update('complianceRegionsText', v)} placeholder="Comma-separated regions"/>
  </div><div className="actions"><button onClick={() => saveScenario(false)} disabled={loading.createScenario}><Save size={16}/>Save Scenario</button><button className="primary" onClick={() => saveScenario(true)} disabled={loading.createScenario}><Activity size={16}/>Save and Run Impact Analysis</button></div></section>;
}
...
function ActivityTrend({ rows = [] }) {
  const trend = rows.slice().reverse().map((row, index) => ({ label: row.scenarioName || `Activity ${index + 1}`, value: index + 1, status: row.recommendationStatus }));
  return <section className="panel chartPanel"><h2>Recent Scenario Activity</h2>{trend.length === 0 ? <p>No activity yet.</p> : <><LineChart rows={trend}/><div className="activityList">{rows.slice(0, 4).map((item) => <div className="listRow" key={item.scenarioId + item.timestamp}><strong title={item.scenarioName}>{item.scenarioName}</strong><span>{item.recommendationStatus}</span></div>)}</div></>}</section>;
}
